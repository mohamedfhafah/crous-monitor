...
class CROUSMonitorMain:
    ...
    def store_listings(self, listings: List[Dict], crous_region: str) -> Tuple[List[Dict], List[Dict]]:
        ...
        cursor.execute('''
            ...
            AND datetime('now', '-{} hours') > last_seen
        '''.format(grace_period_hours), (crous_region,))
...
"""
    ...
    async def run_monitoring_cycle(self):
        ...
        for url in self.config['scraping']['urls']:
            try:
                ...
                if 'crous-aix-marseille.fr' in url:
                    listings = self.scraper.scrape_crous_aix_marseille(url)
                else:
    ...
    async def start_monitoring(self):
        ...
        schedule.every(interval).seconds.do(self._schedule_monitoring_cycle)

        logger.info(f"⏰ Monitor scheduled to run every {interval} seconds")
        logger.info("📊 Hourly status reports will be sent to Telegram")
    ...
    def _schedule_monitoring_cycle(self):
        """Schedule a monitoring cycle as an asyncio task."""
